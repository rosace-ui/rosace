...
//! per-app generated (`tzr new`'s `ffi_rs`) so an app that never asks for
...
/// framework-global atoms (see `tezzera_core::platform::PLATFORM_ATOM_ID`
/// `0xFFFD`, `SAFE_AREA_ATOM_ID` `0xFFFE`, `tezzera_theme`'s `THEME_ATOM_ID`
/// `0xFFFF`) — well clear of the auto-incrementing per-component atom ids
/// (`tezzera_state::atom_id_gen`, starts at 1).
...
mod tests {
    ...
    // same reasoning as `tezzera-cli`'s `CWD_LOCK` (`test_support.rs`).
