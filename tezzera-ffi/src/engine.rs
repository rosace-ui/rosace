...
//!
//! Wraps `tezzera::FrameEngine` (build/paint/input) + `GpuPresenter`
//! (`tezzera-compositor`) + the base/overlay `SkiaCanvas`es, replaying the
//! same per-frame sequence `tezzera-platform`'s winit loop runs (see
//! `tezzera-platform/src/app.rs`'s `RedrawRequested` handler) but driven by
...
use tezzera_compositor::{CompositorLayer, GpuPresenter, LayerRect};
...
pub struct Engine {
    frame_engine: tezzera::FrameEngine,
    presenter: GpuPresenter,
    canvas: SkiaCanvas,
    overlay_canvas: SkiaCanvas,
    scroll_layers: Vec<tezzera_platform::ScrollLayer>,
    pending_events: Vec<tezzera_platform::InputEvent>,
    width: u32,
    height: u32,
    scale: f32,
}
...
impl Engine {
    ...
    pub fn init(root: Box<dyn Component>, theme: ThemeData, surface: RawSurface) -> Option<Box<Engine>> {
        ...
        Some(Box::new(Engine {
            frame_engine: tezzera::FrameEngine::new(root, font),
            presenter,
    ...
    /// `tezzera-platform/src/app.rs`'s `RedrawRequested` handler runs.
    pub fn frame(&mut self) {
        ...
        let refreshed = tezzera_platform::take_scroll_layers();
