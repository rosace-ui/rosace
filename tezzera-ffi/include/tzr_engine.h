/* tzr_engine.h — TEZZERA native-host FFI boundary (D106 Phase 24 Step 1).
 *
 ...
 * `tezzera-ffi/examples/ios_stub.rs` for the reference implementation every
 ...
 * Layout must match `tezzera_ffi::event::TzrInputEventFfi` exactly. */
typedef struct {
    uint32_t kind;
    float x;
    float y;
