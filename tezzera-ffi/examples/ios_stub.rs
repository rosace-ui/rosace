...
//!
//! This is NOT shipped code — it exists to prove that `tezzera-ffi`'s safe
...
//! call it. Phase 24 Step 2's `tzr new` codegen will generate the real
//! per-app equivalent of this file (with the app's actual root `Component`
//! in place of `StubRoot`) as `src/ffi.rs`.
//!
//! Build: `cargo build -p tezzera-ffi --example ios_stub --target
