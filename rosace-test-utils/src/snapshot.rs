...
mod tests {
    ...
    fn snapshot_assert_saves_when_no_baseline() {
        let dir = std::env::temp_dir().join("rosace_snapshot_test");
    ...
    fn snapshot_assert_passes_identical() {
        let dir = std::env::temp_dir().join("rosace_snapshot_test2");
        let _ = fs::remove_dir_all(&dir);
        let sa = SnapshotAssert::new().with_dir(&dir);
    ...
    fn snapshot_assert_with_threshold() {
        let dir = std::env::temp_dir().join("rosace_snapshot_test3");
        let _ = fs::remove_dir_all(&dir);
