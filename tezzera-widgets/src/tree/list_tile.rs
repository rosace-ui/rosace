...
/// A vertically stacked list of [`ListTile`]s (or any widgets).
pub struct ListView {
...
impl ListView {
    pub fn new() -> Self { Self { children: Vec::new(), item_height: 48.0 } }
    pub fn item_height(mut self, h: f32) -> Self { self.item_height = h; self }
    pub fn tile(mut self, t: ListTile) -> Self { self.children.push(Box::new(t)); self }
    pub fn item(mut self, w: impl Widget + 'static) -> Self { self.children.push(Box::new(w)); self }
}
...
impl Widget for ListView {
    fn layout(&self, ctx: &LayoutCtx) -> Size {
        let constraints = ctx.constraints;
        let total_h = self.children.iter()
            .map(|c| c.layout(ctx).height)
            .sum();
        Size { width: avail_w(constraints), height: total_h }
    }

    fn paint(&self, ctx: &mut PaintCtx) {
        let mut y = ctx.rect.origin.y;
        let w = ctx.rect.size.width;
        let x = ctx.rect.origin.x;
        for child in &self.children {
            let c = Constraints::loose(w, f32::INFINITY);
            let size = child.layout(&ctx.layout_ctx(c));
