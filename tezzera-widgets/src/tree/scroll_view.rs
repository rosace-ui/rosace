...
use tezzera_scroll::{ScrollController, ScrollPhysics, ScrollStyle};
...
pub fn resolve_physics(theme: &tezzera_theme::ThemeData, explicit: Option<ScrollPhysics>) -> ScrollPhysics {
    explicit
        .or_else(|| theme.ext::<ScrollStyle>().map(|s| s.physics))
        .unwrap_or_else(|| ScrollStyle::default_for_platform(tezzera_core::use_platform()))
}
...
impl ScrollView {
    ...
    fn paint_base(&self, ctx: &mut PaintCtx, child_size: Size) {
        ...
        if let Some(ctrl) = &ctrl {
            ...
            // further, TEZZERA's own spring-back tried to recover, the next
            ...
            // alone, so the only robust fix is to not double up: TEZZERA's
            ...
            // `tezzera_platform::InputEvent::Scroll`, which doesn't carry
