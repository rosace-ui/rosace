//! `tzr devices` — lists real run targets across platforms, Flutter-`flutter
//! devices`-style. Each entry's `id` is exactly what `tzr run --device <id>`
//! consumes (an iOS simulator UDID, an Android adb serial) — this command
//! and `tzr run`'s `--device` handling are two views of the same data, not
...
pub fn print_help() {
    println!("tzr devices — list available run targets across platforms");
    println!();
    println!("USAGE:");
    println!("  tzr devices");
    println!();
    ...
    println!("what `tzr run --device <id>` expects — copy it directly.");
}
...
/// entries (that's what `tzr doctor` is for diagnosing, not this command).
