//! `tzr bundle-id [<new-id>]` — read or update the app's bundle/package
//! identifier.
//!
//! `tzr new` writes the bundle id into `tzr.toml` (the single source of
...
//! `macos/Info.plist`). Editing `tzr.toml` by hand would silently desync
...
impl BundleIdOptions {
    pub fn from_args(args: &[String]) -> Result<Self, String> {
        ...
        let new_id = match args.first() {
            ...
            Some(other) => return Err(format!("unexpected flag '{}'. usage: tzr bundle-id [<new-id>]", other)),
            None => None,
        };
        Ok(Self { new_id })
    }
...
pub fn print_help() {
    println!("tzr bundle-id [<new-id>] — read or update the app's bundle identifier");
    println!();
    println!("USAGE:");
    println!("  tzr bundle-id              Print the current bundle id");
    println!("  tzr bundle-id <new-id>     Set a new bundle id everywhere it's embedded");
    println!();
    println!("Updates tzr.toml plus any of these that already exist:");
    println!("  ios/Info.plist");
    println!("  ios/App.xcodeproj/project.pbxproj");
    println!("  macos/Info.plist");
    println!();
    println!("EXAMPLES:");
    println!("  tzr bundle-id");
    println!("  tzr bundle-id com.example.myapp");
}

pub fn run(opts: BundleIdOptions) -> Result<(), String> {
    if !Path::new("tzr.toml").exists() {
        return Err("no tzr.toml here — run `tzr bundle-id` from an app directory".to_string());
    }
    ...
    if updated.is_empty() {
        println!("  (no generated platform files found to update — only tzr.toml would change, but see error above)");
    } else {
        for f in &updated {
            println!("  updated {}", f);
        }
...
fn read_tzr_toml_bundle_id() -> Result<String, String> {
    let s = fs::read_to_string("tzr.toml").map_err(|e| format!("cannot read tzr.toml: {}", e))?;
    for line in s.lines() {
    ...
    Err("tzr.toml has no bundle_id line".to_string())
}

fn write_tzr_toml_bundle_id(new_id: &str) -> Result<bool, String> {
    let content = fs::read_to_string("tzr.toml").map_err(|e| format!("cannot read tzr.toml: {}", e))?;
    ...
    if !found {
        return Err("tzr.toml has no bundle_id line to update".to_string());
    }
    fs::write("tzr.toml", out).map_err(|e| format!("cannot write tzr.toml: {}", e))?;
    Ok(true)
}
...
mod tests {
    ...
    fn replace_between_single_occurrence() {
        let input = "<key>CFBundleIdentifier</key><string>dev.tezzera.old</string>";
    ...
    fn replace_between_multiple_occurrences() {
        let input = r#"PRODUCT_BUNDLE_IDENTIFIER = "dev.tezzera.old";
some other line;
PRODUCT_BUNDLE_IDENTIFIER = "dev.tezzera.old";"#;
        ...
        assert!(!out.contains("dev.tezzera.old"));
    }
