...
/// Result of running a tzr workspace command.
#[derive(Debug, Clone)]
...
/// `tzr check` — runs `cargo check --workspace`
pub fn run_check(verbose: bool) -> CommandResult {
    ...
    run_cargo(&args, "tzr check")
}

/// `tzr test` — runs `cargo test --workspace`
pub fn run_test(filter: Option<&str>) -> CommandResult {
    ...
    run_cargo(&args, "tzr test")
}

/// `tzr lint` — runs `cargo clippy --workspace -- -D warnings`
pub fn run_lint() -> CommandResult {
    run_cargo(&["clippy", "--workspace", "--", "-D", "warnings"], "tzr lint")
}

/// `tzr fmt` — runs `cargo fmt --workspace --check`
pub fn run_fmt_check() -> CommandResult {
    run_cargo(&["fmt", "--workspace", "--check"], "tzr fmt")
}
...
mod tests {
    ...
    fn command_result_success_summary() {
        let r = make_result(true, 0, "tzr check");
        assert!(r.summary().contains("completed"));
        assert!(r.summary().contains("tzr check"));
    }

    #[test]
    fn command_result_failure_summary() {
        let r = make_result(false, 1, "tzr lint");
        assert!(r.summary().contains("failed"));
        assert!(r.summary().contains("tzr lint"));
    }
