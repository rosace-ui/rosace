//! Web (WASM) platform backend for TEZZERA.
//!
//! Activated automatically when compiled for `wasm32-unknown-unknown`.
//! Uses a `<canvas id="tezzera-canvas">` element in the host page.
...
pub mod web_app {
    ...
    ///
    /// Finds the `<canvas id="tezzera-canvas">` element, sets its dimensions,
        ...
        F: FnMut(&mut SkiaCanvas, &[InputEvent]) + 'static,
    {
        ...
        let canvas = document
            .get_element_by_id("tezzera-canvas")
            .expect("no #tezzera-canvas element in page")
            .dyn_into::<HtmlCanvasElement>()
            .expect("#tezzera-canvas is not a <canvas> element");
