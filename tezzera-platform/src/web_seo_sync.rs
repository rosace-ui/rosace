//! D107 Phase 25 Step 4 — runtime shadow-DOM sync. Keeps the `#tzr-seo`
//! shadow root (Step 3's build-time `<template shadowrootmode="open">`, or
//! a fresh one attached on the fly if that step never ran — e.g. `tzr dev`)
...
//!
//! Called from `tezzera`'s `App::launch` closure (Layer 7 — has access to
...
use tezzera_core::SemanticNode;

const HOST_ELEMENT_ID: &str = "tzr-seo";
...
pub fn sync(tree: &SemanticNode) {
    let html = tezzera_web_seo::render_html(tree);
    ...
    SHADOW_ROOT.with(|cell| {
        ...
        // If there's still no shadow root (e.g. #tzr-seo isn't in this
        // page's HTML at all — an app that isn't using tzr new/tzr build's
...
/// otherwise attaches a fresh one (e.g. `tzr dev`, which doesn't run the
