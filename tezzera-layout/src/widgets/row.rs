...
use crate::widgets::flex::layout_row;

/// A widget that arranges its children horizontally (left to right).
///
...
impl Row {
    /// Create a new `Row` with default alignments and zero spacing.
    pub fn new() -> Self {
        Self {
            children: Vec::new(),
    ...
    /// Set how children are distributed along the horizontal main axis.
    pub fn main_axis_alignment(mut self, a: MainAxisAlignment) -> Self {
        self.main_axis_alignment = a;
        self
    }

    /// Set how children are aligned on the vertical cross axis.
