...
//! `Cargo.toml` description for the two consumers (`rsc build --target
//! web`, and `rosace-platform`'s wasm32-gated runtime module) and why
...
/// literal HTML fragment `rsc build --target web` embeds directly in a
