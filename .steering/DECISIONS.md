...
**Remaining**: route `ScrollView::live` through this — the last piece (today it paints into the base canvas; the hit-testing prereq is now done, but it's still a demo-wide blast radius since all routes wrap `ScrollView`); content > `MAX_TL_DIM` (4096px) re-render windowing.
