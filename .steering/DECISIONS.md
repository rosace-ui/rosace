...
**Status**: IN PROGRESS — Steps 1-2 landed 2026-07-08: the native-bridge FFI boundary (`tezzera-ffi`) and a real `tzr new`-generated `.xcodeproj` + Swift `AppDelegate`/`SceneDelegate`/`EngineViewController` host, both verified with real `xcodebuild` builds and iOS Simulator runs (see `.steering/PHASE_24.md`). Steps 3-5 (Android Gradle/JNI generation, `tzr run` toolchain integration, capability proof) remain. Full plan in `.steering/PHASE_24.md`.
