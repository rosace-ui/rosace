...
**Status**: STARTED 2026-07-15 — Step 1 scope EXPANDED user-directed same day: + layout `Table`, `InteractiveViewer` (2D pan/zoom plane, gesture-driven), `Grid` staggered/bento modes, `Dialog` modal/non-modal/full-page variants, `Drawer`/`Sheet` full-screen+scrollable, and a universal D094 customization sweep over the existing widget set. `nav_rail` name kept (Material's own `NavigationRail` term). See PHASE_32.md Step 1 — see `.steering/PHASE_32.md`.
...
### D123 — Observability & DevTools track, and the core-extensibility seams it forces (raised + scoped 2026-07-16)

**Status**: SCOPED. Foundation (event classification + always-on flight recorder, the disaster fix) LANDING 2026-07-16; the O1-O7 phase track (`.steering/PHASE_OBSERVABILITY.md`) is the roadmap.
**Decision**: Build an in-app observability + DevTools experience as its own phase track (O1-O7), and treat it as the FORCING FUNCTION that matures `rosace-core` from a mature *app framework* into an adaptable *platform* others build on. Two things are true at once and this decision links them: (1) the tooling can't exist without core exposing read-only introspection seams (the render-tree/state inspectors), so building it hardens the core; (2) the tooling must NOT require architectural change — the framework's immutable-value-widget model stays. Concretely that means DevTools is READ-mostly: inspect the render tree (rects, sizes, semantics, hits), stream meaningful trace events, profile frames, visualize layout/overflow — all additive read APIs over structures that already exist (`RenderTree`, the trace event vocabulary, the atom system with `AtomWrite.by` causality).
**Reason (why now)**: A capability audit (2026-07-16) found the core is mature for BUILDING apps but not for being built ON: three "give-out" gaps — (a) no *unified* capability/extension idiom (persistence uses a trait, FFI capabilities use the queue+atom shape, platform/safe-area/IME use bare global fns — three idioms); (b) no widget introspection/override seam; (c) thin embedding story. The scoped roadmap's feature phases are nearly done, and the last few sessions (resize-flicker hunt especially) were slow precisely because there was no in-app frame/present timeline. Tooling is now the highest-leverage investment.
**The hard-won constraint (user-directed, from a past disaster)**: an earlier tracing attempt HUNG the app and spammed a log line every frame. Root cause: nothing classified events by frequency, so a subscriber received the per-frame firehose (`AtomRead`/`FrameStart`/`FrameEnd`/`PaintRegion`/`LayoutStart`). THE governing rule for this whole track: **tracing is EVENT-DRIVEN, never frame-driven — meaningful state transitions only, captured into a bounded ring buffer at near-zero cost, and NO visible sink (console/overlay) or the default recorder ever receives high-frequency events.** Enforced structurally by `RosaceTrace::is_high_frequency()` classification (this decision's first landed piece), not by discipline.
**Deliberately deferred (user-directed)**: LIVE PROPERTY EDITING of a selected widget (changing a hard-coded `.padding(8)` at runtime). It fights the immutable-value-widget model — a widget is rebuilt fresh every frame from `build()`, so there is nothing persistent to "edit". Atom-backed props can already be changed via the state inspector; arbitrary static props would need a per-node debug-override seam that only makes sense once HOT RELOAD/HOT RESTART (D102) exists to re-run `build()` with patched values. So: property editing is a NAMED FUTURE IMPROVEMENT gated on D102, NOT part of O1-O7. DevTools ships read + select + inspect + profile; edit comes with hot reload.
**Also deferred but named (the three give-out gaps)**: unifying the capability/extension idiom (subsumes D122 platform channels) and the embedding entry point are real core-maturity work but NOT required for O1-O7's read-only tooling — scoped as follow-on once the tooling proves which seams matter. Only the read-only introspection seam is built now (it's what DevTools needs and it's non-invasive).
**Affects**: `rosace-trace` (event classification, always-on filtered flight recorder), `rosace-widgets` (`RenderTree` inspection snapshot — O2), `rosace-devtools` (the inspectors, currently data-only structures, get wired to a real overlay), `rosace` (`App::launch` installs the flight recorder by default in debug instead of the print-everything console path).
**Relates to**: D102 (hot reload — gates live property editing), D122 (platform channels — part of the deferred capability-unification), Known Issue #17 (the debugging this tooling would have accelerated), the existing `rosace-devtools` inspectors (built-but-never-wired, finally getting their UI).

### D124 — Custom shader materials: `ShaderPaint`, the widget→theme→none material cascade, backdrop sampling (planned 2026-07-18)

**Status**: PLANNED — full step plan in `.steering/PHASE_33.md`.
**Decision**: Build the `ShaderPaint` widget D109 promised (own type, not a `CustomPaint` mode-switch — that call was already made in D109 and stands), and around it a **material system**: a single `ShaderMaterial { pipeline, uniforms, fallback }` value usable at two cascade levels — per-widget-instance (`.material(...)` builder on surface widgets) and app-global via D105 type-keyed theme extensions (`CardMaterial(ShaderMaterial)` etc.) — resolved instance → theme → none by ONE shared resolver mirroring `resolve_physics`'s existing explicit→ext→default precedent. Plus **backdrop sampling** as a declared pipeline capability (`ShaderSpec::with_backdrop()`): the compositor generalizes D-DEF-012's already-shipped scene-texture indirection so ANY registered pipeline can sample the scene behind its rect — the piece that makes Liquid-Glass-style materials expressible through the general registry instead of only via the hardcoded `BackdropBlur` command.
**Reason**: (1) `ShaderPaint` is the recurring built-but-never-wired pattern again — D109 built the whole registry/`ShaderFill`/`shader_fill()` plumbing and the widget never landed. (2) User goal: 2026-era app looks (per-surface custom materials, glassmorphism) built ON rosace, controllable app-wide from one registration. (3) Research into Apple's Liquid Glass showed the critical missing capability is backdrop access — glass refracts what's BEHIND it; uniforms-only shaders structurally can't do glass — and that Apple ships a curated declarative style, not raw shader authorship, which motivates the starter material library alongside the raw WGSL door.
**Key constraints**: "global" is per WIDGET KIND (type-keyed, third-party-extensible — a Card can be glass while Buttons stay solid), never one shader for all surfaces. All additive: zero appearance/perf change for apps that never set a material; `BackdropBlur`'s dedicated fast path untouched. Honest degradation: no-GPU paths (softbuffer/web) render the material's `fallback` color, never a silent hole. Animated materials are event-driven (`request_animation`), never free-running (D123's rule). Pipelines compile eagerly at registration (D109's Impeller lesson).
**Deferred, named**: GlassEffectContainer-style adjacent-shape merging (future phase); interaction-reactive materials (hover/press as standard uniforms — hooks exist); shader hot-reload (rides D102); web (gated on D-DEF-011).
**Affects**: `rosace-shader` (ShaderMaterial, materials library, `with_backdrop`), `rosace-widgets` (ShaderPaint, MaterialKey + resolver, `.material()` on surface widgets), `rosace-compositor` (backdrop bind-group for flagged pipelines), gallery/demos.
**Relates to**: D109 (the registry this rides), D105 (theme-ext mechanism), D-DEF-012 (scene capture being generalized), D115 (icon registry — the extensibility bar Step 5 mirrors), D094 (builder vocabulary).

---

### D125 — Template descriptor data model (D103 Tier 1 first artifact, 2026-07-21)

**Status**: IN PROGRESS — implements D103 (Tier 1 rollout steps 2–3). Data model + template/widget registries + macro two-mode codegen + interpreter LANDED 2026-07-21 in `rosace-widgets::template` and `rosace-macros`; the watcher/transport (step 4) + macro hole-filler array + live swap demo are what remain.
**Release-path decision (was OPEN in D103) — RESOLVED to Option A (2026-07-21)**: `view!` in release emits ONLY builder calls (zero template/interpreter machinery in the shipped binary); the descriptor is emitted only under the `rsc-hot` feature (`rosace-macros/rsc-hot`, re-enabled by `rosace/rsc-hot`, on for `rsc dev`). Gated in the macro via `cfg!(feature = "rsc-hot")`. Divergence risk (dev interpreter vs release builder) is guarded by the interpreter test suite (step 3). Chosen over Option B (always-template) for the optimized builder/GPU paint path. Literal props → `StaticValue`, non-literals → positional `Hole` (props-before-children traversal order); site key from `file!/line!/column!`.
**Decision**: The concrete shape of the descriptor D103 calls for: `Template { key: TemplateKey, root: TemplateNode, hole_count }`, `TemplateNode { widget: String, props: Vec<(String, PropValue)>, children }`, `PropValue = Static(StaticValue) | Hole(usize)`, `StaticValue = Bool|Int(i64)|Float(f64)|Str`, `TemplateKey { file: String, line, col }`. Widget/prop **names are strings** (registry keys, resolved at inflate time — the node knows nothing about painting); dynamic `{expr}`s are **positional holes**; only primitive literals travel as `StaticValue` (everything else is a hole). Lives in `rosace-widgets` (Layer 6) alongside the interpreter/registry that will consume it; re-exported through `rosace::widgets`.
**Reason**: This is the contract the macro writes and the interpreter reads — it must exist before either. Positional holes are all hot reload needs (dev recompiles the same source → stable hole order). `hole_count` is the necessary first gate of D103's slot-signature boundary (a changed count = added/removed compiled logic → escalate to Tier 2/0).
**Serde/wire (named deferral)**: types are plain POD (String names, primitive-only `StaticValue`, no lifetimes) so `#[derive(Serialize, Deserialize)]` drops on cleanly when the transport step needs JSON — but `serde` is **NOT** added to the workspace here. Consistent with D121: a new dependency is its own decision, added at the step that exercises it (rollout step 4), not smuggled in with the data model.
**Deferred, named**: name-based hole binding (`"onSave"` → handler) — what SDUI needs; positional suffices for hot reload. The JSON wire form + `serde` dep. `RosaceTrace` inflate events attach to the interpreter (step 3), not this inert data model.
**Safety requirement (locked, not yet built)**: `hole_count` equality is only the first gate. Before inflating a pushed template the interpreter MUST also check each slot's type/arity **signature** against the running binary's hole at that index — else a stale/reordered template drops a closure into a `bool` slot and builds a corrupt widget instead of escalating. Per-slot signature (widget+prop+value-kind) lands with the diff step (rollout step 4); a mismatch at ANY slot → escalate, never inflate. See HOT_RELOAD.md "Slot-signature safety".
**Interpreter + widget registry (step 3, 2026-07-21)**: `inflate(&Template, &[Box<dyn Any>]) -> Result<Box<dyn Widget>, InflateError>` rebuilds the tree via the SAME constructors as the builder (verified by layout-equivalence tests: inflated tree measures identically to hand-written builder, static + hole-by-index). Registry = global `OnceLock<RwLock<HashMap<String, BuildFn>>>` mirroring D115's icon registry; each widget registers ONE build closure `(props, children) -> Box<dyn Widget>` (no widget downcasting needed since builders are move-based — the closure holds all type knowledge). Third parties `register_widget`. Errors escalate (`UnknownWidget/UnknownProp/PropType/UnexpectedChildren/HoleOutOfRange`), never inflate garbage. Value holes only; handler/closure holes deferred (need a typed handler-registry — the name-binding extension). **Ergonomic layer**: a public `FromProp` trait (the typed edge from untyped props/holes to builder args; impls for primitives + String, third-party-extensible) + an `inflatable!` macro that generates a build closure from a `"prop" => setter: Type` table — so registration is a few lines, not a hand-written closure, and is the single declared prop-schema source (future `#[derive(Widget)]` / IDE schema feed from the same shape; a real derive needs impl-level introspection macros aren't given).
**Affects**: `rosace-widgets` (new `template` module: descriptor + template registry + widget registry/interpreter), `rosace-macros` (two-mode `view!`), `rosace` (`rsc-hot` feature, `view!` re-export).
**Relates to**: D103 (the parent decision), D098–D100 (Widget protocol the registry inflates into), D115 (icon registry — the registry/extensibility pattern mirrored), D121 (the dependency-discipline precedent for the serde deferral).

---

### D126 — winit dropped for web; own rAF-driven frame loop (2026-07-30)

**Status**: LANDED and browser-verified 2026-07-30.
**Decision**: Web no longer uses winit at all. `PlatformWindow::run_layered` cfg-splits: desktop keeps the winit `EventLoop`/`ApplicationHandler` path (`AppState`, now `#[cfg(not(target_arch = "wasm32"))]`) unchanged; web calls a new `run_web_native`, which creates the `<canvas>` itself, builds the wgpu surface directly from it (`rosace_compositor::GpuPresenter::new_async_canvas` — the compositor's surface-creation step was split out of its shared build so a canvas and a winit window can both feed it), wires our own canvas-level input listeners (`wire_web_input`, already landed pre-D126 for the winit-web bridge and unchanged here), and drives frames from a **self-rescheduling `requestAnimationFrame` callback** that calls the paint+present pipeline directly — never through `window.request_redraw()`.
**Reason**: winit-web's `request_redraw` is coalesced/unreliable regardless of how it's triggered — tried and rejected: calling it from `about_to_wait` (the loop parks and doesn't wake reliably from the `request_frame` proxy), calling it from inside the redraw handler itself (coalesced away, the same gotcha D076's docs already warned about on macOS), and an external `requestAnimationFrame` pump calling it every tick (still coalesced — measured 2 drains for 16 queued input events). The practical symptom: the web input bridge's `WEB_INPUT_QUEUE` rarely drained, so typing/clicks worked for a moment after page load and then silently died once the app went idle. The only fix that worked is to stop asking winit to redraw and call the frame logic directly — this validates the standing project position that winit is a desktop-only convenience, not a cross-platform requirement (see `feedback_winit_desktop_only` in project memory).
**Side effect found and fixed in the same pass**: focusing a text field crashed the wasm module outright. The long-press-select timer in `rosace/src/engine.rs` (armed on `MouseDown` over an editable) called `std::thread::spawn`, which is unsupported on `wasm32-unknown-unknown` (no threads → hard abort, not a catchable panic). The same pattern existed in `Toast`/`Snackbar`'s auto-dismiss timers in `rosace-widgets`. Fixed with a new `rosace_state::fire_after_ms(ms, f)` primitive (a timer thread on native, `setTimeout` via web-sys on wasm) — all three call sites now go through it.
**Cleanup (same day)**: `AppState`/`ApplicationHandler` and their winit/std imports are now `#[cfg(not(target_arch = "wasm32"))]`; every wasm-only branch that used to live inside them (`PENDING_PRESENTER`'s async-GPU hand-off, the `WEB_INPUT_QUEUE` drain inside `AppState::redraw`, the canvas-creation/rAF-pump/async-GPU-init in `resumed()`) is deleted rather than left dead. `rosace-platform/src/web.rs` (the original pre-winit single-frame `putImageData` MVP, already dead since the winit-web era and doubly so now) was also removed — no callers anywhere.
**Verified**: full workspace `cargo check` clean on host, `wasm32-unknown-unknown`, `aarch64-apple-ios-sim`, and `aarch64-linux-android` (zero warnings). Browser-tested against the `gallery_all` example (`rosace-examples`): render (crisp text, correct sRGB colors), click, drag-to-pan scrolling with momentum, and text typing all confirmed working end-to-end after the change, console clean.
**Known gap, not regressed by this change**: mouse-wheel scroll events are received by `wire_web_input` but don't yet move ROSACE's own scroll views on web — pre-existing, tracked separately, not part of this decision.
**Affects**: `rosace-platform` (`src/app.rs`, new `run_web_native`/`WebState`), `rosace-compositor` (`GpuPresenter::new_async_canvas` + the `make_instance`/`build_from_surface` split), `rosace` (`src/engine.rs` long-press timers), `rosace-widgets` (`Toast`/`Snackbar` timers), `rosace-state` (new `fire_after_ms` in `frame_scheduler.rs`).
**Relates to**: D-DEF-011 (Web GPU presenter — the wgpu-on-wasm work this builds directly on top of), D076 (the redraw-coalescing gotcha this rediscovered on web), D106 (mobile's precedent for bypassing winit entirely via a native host loop).

---

### D127 — Platform Channel: generic native↔Rust method-call bridge (2026-07-31)

**Status**: LANDED (core primitive + camera/push migration + generator wiring), same day.
**Decision**: A named, bidirectional method-call bridge to native platform code, mirroring Flutter's `MethodChannel` concept — generalizes the request/result shape `rosace-ffi::capability`'s camera/push proof already established (that module's own doc comment: "a second capability would follow this exact same shape, not a new architecture"). Two independent call directions, split by single responsibility into separate files (`rosace-ffi/src/platform_channel/{dispatch,outgoing}.rs`):
- **Sync (native calls Rust)** — `dispatch.rs`: a handler registry (`set_method_call_handler`/`dispatch`), one blocking FFI call answered inline (the same shape `rsc_engine_init` already uses, walked in reverse). For fast work only.
- **Async (Rust calls native)** — `outgoing.rs`: native may take arbitrarily long (a system dialog, a slow SDK call), so the call is queued (`invoke_method`/`take_outgoing_calls`, mirroring `take_camera_request`'s per-frame-poll shape) and the result delivered back later via a real `rosace_state::Atom` (`report_call_result`/`report_call_error`) — not a new "async" concept; the exact same atom-write-wakes-subscribers mechanism `rosace-net::query`'s background-thread case already uses, just triggered by a native callback instead of a thread completing.
**Wire format**: `serde_json` — a new, named dependency for `rosace-ffi`. This codebase has otherwise deliberately avoided `serde` (`ureq`/`tungstenite` were picked specifically to dodge an async-runtime cascade), but that reasoning is about avoiding a *runtime*, not a data-encoding library — `serde_json` touches neither threading nor control flow. Native platforms already speak JSON natively (`JSONSerialization`/`org.json`) regardless of Rust-side tooling, so hand-rolling an alternative would just be a worse, less-tested parser for no benefit.
**"One root" unification (same day)**: `capability.rs`'s camera/push permission *discovery* was migrated onto this primitive — `request_camera`/`request_push_permission` now call `invoke_method` internally, and native's tick loop became the single generic poller (`take_outgoing_calls`), replacing the old dedicated `rsc_push_permission_take_request`. Result-reporting (`report_camera_result`/`report_push_result`/`report_push_token`/`report_push_notification`) deliberately stayed plain setters — a singleton capability (never more than one camera permission in flight) doesn't need `call_id` correlation, only an app-defined channel with genuinely concurrent calls does.
**Breaking change, deliberate**: `rsc_push_permission_take_request` is retired from `rsc new`'s generated FFI surface, replaced by `rsc_platform_channel_take_outgoing`/`_report_result`/`_report_error`/`_dispatch` + `rsc_string_free`. Pre-1.0, done on explicit direction ("port everything… one root") rather than papering over two poll mechanisms with a compatibility shim (a drain-and-requeue hybrid was considered and rejected — it fights the design instead of fitting it). iOS's existing push-permission flow now discovers via the generic poll, unchanged otherwise (`requestPushPermission()`/`rsc_push_permission_report_result` untouched). Android push notifications were never wired into the generator before this change at all (a separate, pre-existing gap) — the generic bridge now exists there too, but `"rosace/push"` is deliberately left unrecognized in Android's poll loop (building real `POST_NOTIFICATIONS` support is new scope, not migration, and fabricating a fake "granted" response would be dishonest) — a named follow-up, tracked in `.steering/HOLES_REGISTER.md`.
**Deferred, named**: native library packaging (`RosaceActivity`/`RosaceViewController` as a real distributable Swift Package/AAR so generated apps are a thin subclass instead of carrying raw FFI-wiring source) — explicitly held back; for now the generated glue stays a referenced, editable class in the project, not an opaque package, so it can still be customized. Event channels (a continuous native→Rust stream, e.g. live sensor data) — method channels (call → one response) only for v1, matching how every other phase here ships the smallest complete piece first.
**Affects**: `rosace-ffi` (new `platform_channel` module, `Cargo.toml` serde/serde_json dep, `capability.rs` rewrite), `rosace-cli` (`ffi_rs`/iOS Swift/Android Kotlin templates in `commands/new.rs`, generated app `Cargo.toml`'s new `serde_json` dep), `rosace-ffi/examples/ios_stub.rs` (kept in sync as the reference stub).
**Relates to**: D106 (the native-host FFI boundary this builds on), D110 (push notifications — the second capability this generalizes). Same session, same principle ("backport a proven pattern into the actual generator, not just one example app"): the mobile soft-keyboard bridge (iOS `UIKeyInput` + Android `InputConnection`) was also landed into `rsc new`'s templates directly beforehand — undocumented as its own decision entry, noted here for context.

---

### D128 — Video widget + controller architecture (2026-08-04)

**Status**: OPEN (proposed — architecture investigated and written up for review, nothing built yet; see `PHASE_34.md` for the execution plan this unlocks once accepted).

**Question**: `rosace-media::VideoDecoder` is a pure stub (`open()` always returns `MediaError::PlatformUnavailable`). What's the real, buildable architecture for a `VideoPlayer` widget + controller — decode where, deliver frames how, render through which existing (or new) engine primitive — given the project's standing principle of adapting to native/existing capability rather than reimplementing it (`rosace-net`/`rosace-file`'s "no Swift/Kotlin layer to cross where we can help it" philosophy)?

**Researched first, not guessed**: read `rosace-ffi`'s Platform Channel (D127) in full, the camera capability it generalizes (`capability.rs`), `rosace-compositor`'s actual layer/texture pipeline (`lib.rs`, 2979 lines), `DrawCommand`'s full definition and both its CPU and GPU-shapes dispatch paths (`rosace-render/src/canvas.rs`), `rosace-net::use_query`/`LoadState<T>`'s async-resource idiom, and `rosace-state`'s `Atom`/`GlobalAtom` API — see `PHASE_34.md`'s "Why This Phase" for the full findings. Four load-bearing facts came out of that reading, none obvious from the crate names alone:

1. **Platform Channel is one-shot request/response only** — D127 itself names "event channels (continuous native→Rust stream)" as an explicit v1 deferral. There is no existing native→Rust streaming primitive a per-frame video feed could reuse.
2. **`rosace-compositor` has no external-texture-import path** — confirmed by reading the full file (zero hits for anything like `import_texture`). Every pixel that reaches the compositor is either CPU-rasterized by `tiny-skia` or assembled from `CanvasFrameItem`s the compositor's own registered wgpu pipelines draw — there is no way today to hand it an externally-decoded GPU texture directly (the Flutter `Texture`/`TextureRegistry` zero-copy model has no ROSACE equivalent to build on).
3. **The existing "bring your own pixels" primitive is wrong-shaped for video as-is.** `DrawCommand::BlitRgba` → GPU-shapes' `CanvasFrameItem::Image` → `rosace-compositor::ImageQuad` is real and works, but its cache key is a **content hash of the pixel bytes** (`blit_key`, `rosace-render/src/canvas.rs`), read once per distinct hash and reused every subsequent frame — correct for a static image, but a video frame is different content every single frame by definition, so routing raw frames through it as-is would hash and re-upload the full frame every tick, defeating the cache it's built around.
4. **No raw-pixel-buffer FFI convention exists to reuse for frame delivery either.** `rsc_engine_frame` takes only `*mut Engine` — ROSACE's own rendered output never crosses the FFI boundary as bytes at all; `wgpu` draws directly into the native-provided `CAMetalLayer`/`ANativeWindow` surface. So there's no existing "hand Rust some pixels each frame" channel to piggyback on for mobile video frame delivery — it's a genuine gap, not a rediscovery of something already there.

**Decision**: A phased, honestly-scoped v1 that reuses everything reusable and names every real new piece explicitly, rather than reaching for either extreme (hand-rolling a decoder, or claiming a nonexistent shortcut):

- **Decode**: native platform decoders only, never a Rust-side codec — `AVPlayerItemVideoOutput` (iOS/macOS), `ExoPlayer`+`ImageReader` (Android), the browser's own `<video>` element (web, via the existing D107 semantic-DOM-shadow-tree machinery). Desktop (Windows/Linux) is the one honest gap: no equivalent OS-level "give me raw decoded frames" API as clean as AVFoundation/ExoPlayer, and the real Rust options (`ffmpeg-next`, `gstreamer-rs`, `libmpv-rs`) all require a *system-installed* native library plus LGPL/GPL licensing considerations that need their own explicit sign-off — **not decided here**, tracked as its own open sub-decision in `PHASE_34.md`.
- **Frame delivery (v1)**: a CPU pixel copy, not zero-copy — native decode produces a frame, copies it to RGBA bytes, and hands it to Rust through a **new, dedicated, non-JSON FFI entrypoint** (binary pointer + width/height, mirroring how `rsc_engine_input` already takes typed args directly rather than JSON) — control commands (play/pause/seek/volume) stay on the existing Platform Channel (D127) since they're small and infrequent; only bulk frame bytes need a different path.
- **Render path (v1)**: a **new** `DrawCommand::VideoFrame { instance_id, generation, pixels, .. }` — deliberately a sibling of `BlitRgba`, not a reuse of it, because the cache key is `(instance_id, generation)` (a monotonic per-frame counter, the same shape `CompositorLayer`/`OffscreenRef`'s existing `dirty` flags already use elsewhere in the compositor) instead of a content hash — correct invalidation semantics for constantly-changing content, and it keeps `BlitRgba`'s image-cache semantics untouched for actual static images.
- **Controller**: mirrors `rosace-net::use_query`'s exact idiom — per-instance `ctx.state(...)` `Atom`s (not `GlobalAtom`s; multiple concurrent players need independent state), `LoadState<VideoMetadata>` for the async "open" step, an unmount-guard (`alive` flag + `on_unmount`) discarding late native callbacks after teardown, same as `use_query`'s stale-request guard.

**Why not Flutter's zero-copy texture streaming, or Compose's native-view embedding, as the v1 target**: both were read and compared (see `PHASE_34.md`). Flutter's approach (external GPU texture handles sampled directly into the same command buffer as the rest of the frame) is the real performance ceiling, but requires an `import_texture`-class primitive `rosace-compositor` doesn't have — named as future work (`Future-Proofing` below), not built now. Compose's approach (embed the native player view, e.g. `ExoPlayer`'s `PlayerView`, as a sibling in the view hierarchy via `AndroidView` interop) works for Compose because Compose UI still sits inside a real Android `ViewGroup` that the OS itself composites — ROSACE's mobile architecture is different by design (confirmed via `rsc_engine_frame`/`CAMetalLayer` wiring, D106): Rust owns the *entire* GPU surface and draws every pixel itself, there is no OS-level view compositor interleaving ROSACE content with native views today. Native-view z-order embedding is possible (this is literally what Flutter's own "Hybrid Composition" `PlatformView` mode does) but carries Flutter's own documented downsides (clipping/rotation/3D-transform don't compose correctly across the boundary) and would be new mobile-rendering-model surface, not a video-specific decision — named as a possible alternative, not chosen.

**Deferred, named**: zero-copy GPU texture import (`wgpu::Device::create_texture_from_hal` or platform shared-handle import) — the real v2 performance path once there's a second consumer to justify the compositor surface area. DRM (FairPlay/Widevine). Adaptive bitrate (HLS/DASH manifests). Subtitle/caption tracks. Background/audio-only playback lifecycle. Desktop (Windows/Linux) real decode backend — blocked on the `ffmpeg-next`/`gstreamer-rs` licensing + system-dependency decision named above.

**Affects**: `rosace-media` (real `VideoController` + per-platform adapter trait, replacing the stub), `rosace-ffi` (new dedicated frame-delivery FFI export, alongside existing Platform Channel use for control commands), `rosace-render` (new `DrawCommand::VideoFrame`, `CanvasFrameItem::Video`), `rosace-compositor` (new `FrameItem::Video` with generation-keyed cache invalidation), `rosace-widgets` (new `VideoPlayer` widget), `rosace-cli` (`rsc new`'s generated iOS/Android native templates).

**Relates to**: D127 (Platform Channel — control commands reuse it as-is; frame delivery deliberately does not), D109 (GPU-shapes rendering — the new `CanvasFrameItem::Video` variant must fit its existing dispatch shape), D106 (the native-host FFI boundary and single-owned-GPU-surface mobile rendering model this whole decision is scoped around).
