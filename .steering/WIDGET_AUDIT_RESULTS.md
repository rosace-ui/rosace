...
| P2 | Fixed heights that clip scaled text (~25) | **CLOSED** — all of it, including avatar, bottom_nav and list_tile's title-only branch |
| P3 | Hardcoded colours (~20 widgets) | **FIXED** f2f9628 |
| P4 | Touch targets under 44px (~15) | **CLOSED** — all of it. DatePicker cells raised 36 -> 44, guarded by a COMPILE-TIME assert against `MIN_TAP_TARGET` rather than a test, since both are consts. |
...
| P7 | No tests (~20 widgets) | **FIXED for everything that needed it.** Added: `padding`, `list_view`, `positioned`, `wrap`, `scaffold`, `transform_layer`, `responsive`, `row`, `column`, `divider`, `progress_bar`, `circular_progress`, `scroll_view`, `will_pop_scope`, `nav_rail`, `menu`, `list_tile`, `date_picker`. The files still without an in-file module are covered from `engine.rs` (`tooltip`, `pressable`, `semantics`, `hero`, `hero_tag`, `screen_transition_view`, `overlay*`) or are too trivial to pin (`spacer`, `stack`, `aspect_ratio`, `avatar`, `custom_paint`, `app`, `mod`). `focus_api`, `pointer` and `repaint_boundary` have no caller anywhere — the built-but-never-wired set, better deleted or wired than tested. |
