...
mod tests {
    ...
    use crate::event::{ComponentId, TezzeraTrace};
    ...
    impl TraceSubscriber for CountSubscriber {
        fn on_trace(&self, _event: &TezzeraTrace) {
    ...
    fn ring_buffer_evicts_oldest_when_full() {
        ...
        for i in 0..5u64 {
            buf.on_trace(&TezzeraTrace::ComponentUnmount {
                id: ComponentId(i),
                name: "X",
            });
        }
        ...
        assert_eq!(buf.len(), 3);
        let snap = buf.snapshot();
        if let TezzeraTrace::ComponentUnmount { id, .. } = &snap[0] {
            assert_eq!(id.0, 2);
        } else {
