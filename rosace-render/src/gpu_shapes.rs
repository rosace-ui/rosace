//! Built-in GPU shape pipelines (D109 / Phase 27 Step 3) — geometry,
//! uniform layout, and WGSL sources.
//!
//! Lives in `rosace-render` (Layer 4) so `SkiaCanvas::play_picture` can
//! convert shape `DrawCommand`s to GPU quads without an upward dependency;
//! `rosace-shader::builtin` (Layer 5) wraps these WGSL sources into
//! `ShaderSpec`s and registers them, and re-exports everything here for
//! app-facing use.
//!
...
use rosace_core::shader::ShaderUniforms;
use rosace_macros::ShaderUniforms;

/// Raw pipeline ids of the built-in pipelines — inside `PipelineId`'s
/// reserved built-in range (< 0x100). `rosace-shader::builtin` exposes the
/// typed constants.
pub const FILL_RRECT_ID:   u64 = 1;
pub const STROKE_RRECT_ID: u64 = 2;
pub const GRADIENT_ID:     u64 = 3;
pub const ARC_ID:          u64 = 4;
pub const SHADOW_ID:       u64 = 5;
...
/// The five built-in pipelines as `(raw_id, full_wgsl_fragment_source)` —
/// consumed by `rosace-shader::builtin::register_builtins()`, which wraps
/// them in `ShaderSpec`s (a Layer-5 type this crate cannot name).
pub fn builtin_wgsl_sources() -> [(u64, String); 5] {
    let src = |fs: &str| format!("{SDF_LIB}\n{fs}");
    [
        (FILL_RRECT_ID,   src(FILL_RRECT_FS)),
        (STROKE_RRECT_ID, src(STROKE_RRECT_FS)),
        (GRADIENT_ID,     src(GRADIENT_FS)),
        (ARC_ID,          src(ARC_FS)),
        (SHADOW_ID,       src(SHADOW_FS)),
    ]
}
