...
//! `rosace-nav-anim` so `ScreenNav` (the type real apps actually use) can
//! drive it directly — `rosace-nav-anim` already depends on `rosace-nav`,
//! so the dependency could not run the other way without a cycle.
//! `rosace-nav-anim::transition` re-exports these same types so
