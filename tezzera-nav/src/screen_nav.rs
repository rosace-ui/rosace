...
/// 3) — mirrors `tezzera-scroll::ScrollStyle`'s shape exactly. This is the
...
impl NavTransitionStyle {
    ...
    /// either.
    pub fn default_for_platform(platform: tezzera_core::Platform) -> TransitionStyleKind {
        match platform {
            tezzera_core::Platform::Ios
            | tezzera_core::Platform::MacOs
            | tezzera_core::Platform::Android => TransitionStyleKind::Slide,
            tezzera_core::Platform::Windows
            | tezzera_core::Platform::Linux
            | tezzera_core::Platform::Web => TransitionStyleKind::Fade,
        }
    }
}
...
pub struct ScreenNav<R: Clone + Send + Sync + 'static> {
    ...
    /// same reasoning as `tezzera-scroll::ScrollController`'s non-atom
    /// bookkeeping fields.
    ...
    /// per-navigation — matches `tezzera-scroll`'s `resolve_physics` being
...
impl<R: Clone + Send + Sync + 'static> ScreenNav<R> {
    ...
    pub fn new(ctx: &mut Context, initial: R) -> Self {
        ...
        let style = tezzera_theme::provider::use_theme()
            .ext::<NavTransitionStyle>()
            .map(|s| s.style)
            .unwrap_or_else(|| NavTransitionStyle::default_for_platform(tezzera_core::use_platform()));
        Self {
            atom,
            previous,
            transition,
    ...
    fn trigger_if_enabled(&self, style: TransitionStyle) {
        if !tezzera_theme::provider::use_theme().animation.enabled {
            return;
        }
...
mod tests {
    ...
    // phase14_demo at runtime, and by tezzera/src/engine.rs's headless
    ...
    fn default_for_platform_is_slide_on_ios_macos_android() {
        for p in [tezzera_core::Platform::Ios, tezzera_core::Platform::MacOs, tezzera_core::Platform::Android] {
    ...
    fn default_for_platform_is_fade_on_desktop_and_web() {
        for p in [tezzera_core::Platform::Windows, tezzera_core::Platform::Linux, tezzera_core::Platform::Web] {
