...
pub fn print_help() {
    println!("rsc analyze — workspace health: crate count, member list");
    println!();
    println!("USAGE:");
    println!("  rsc analyze [OPTIONS]");
    println!();
    println!("OPTIONS:");
...
pub fn parse_members(toml: &str) -> Vec<String> {
    ...
    inner
        ...
        .filter_map(|s| {
            ...
            // Extract basename from path like "rosace-core" or "../rosace-core"
...
pub fn run_analyze(opts: &AnalyzeOptions) -> Result<AnalyzeReport, String> {
    ...
    let name = extract_workspace_name(&toml)
        .unwrap_or_else(|| "rosace".to_string());
...
"#;
    ...
    fn parse_members_correct_names() {
        let members = parse_members(FIXTURE);
        assert!(members.contains(&"rosace-core".to_string()));
        assert!(members.contains(&"rosace-state".to_string()));
        assert!(members.contains(&"rosace-render".to_string()));
    }
    ...
    fn parse_members_single() {
        let members = parse_members("members = [\"rosace-core\"]");
        assert_eq!(members, vec!["rosace-core"]);
    }
    ...
    fn analyze_report_summary_contains_name() {
        let report = AnalyzeReport {
            workspace_name: "rosace".to_string(),
            member_count:   3,
            members:        vec!["a".to_string()],
        };
        assert!(report.summary().contains("rosace"));
