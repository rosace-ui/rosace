...
/// Options parsed from `rsc build --target <target>`.
#[derive(Debug)]
...
impl BuildOptions {
    ...
    pub fn from_args(args: &[String]) -> Result<Self, String> {
        ...
        let target = match target_str.as_deref() {
            ...
            None => {
                return Err(
                    "--target is required. Usage: rsc build --target desktop | web".to_string(),
                )
            }
        };
...
pub fn print_help() {
    println!("rsc build — release build for a target platform");
    println!();
    println!("USAGE:");
    println!("  rsc build --target <target>");
    println!();
    println!("OPTIONS:");
...
fn build_desktop() -> Result<(), String> {
    println!("Building ROSACE for desktop (release)...");
    ...
    if status.success() {
        ...
        println!("Binary: target/release/rsc");
        Ok(())
    } else {
...
fn build_web() -> Result<(), String> {
    println!("Building ROSACE for web (wasm32)...");
    println!();
    ...
    // this, same as any other project predating a later `rsc new` addition
    // — not an error).
...
/// `seo_extract_rs`'s `SPLIT_MARKER` in `rosace-cli/src/commands/new.rs`,
...
mod tests {
    ...
    fn index_html_contains_canvas() {
        ...
        assert!(html.contains("rosace-canvas"));
